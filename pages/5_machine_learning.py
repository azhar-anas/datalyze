...
if 'current_dataset' not in st.session_state: # Ensure that the dataset has been uploaded
    ...
    if selected_df.isnull().sum().sum() > 0:
        ...
        with st.form(key='data_config_form'):
            ...
            if st.form_submit_button(label='Apply Changes', icon=':material/manufacturing:',use_container_width=True ):
                processed_df = selected_df.copy()
                X = processed_df[features]
                y = processed_df[target]
                if features == []:
                    st.error(':material/error: **Please select at least one independent features**.')
        ...
        with tab1:
            ...
            with st.form(key='model_selection_form'):
                ...
                if st.form_submit_button(label='Train Model', icon=':material/autorenew:',use_container_width=True):
